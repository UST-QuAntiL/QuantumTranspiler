...
class TestTranspilation:
    ...
    def _counts_postprocessing(
        self, counts_all: List[List[Dict[str, Dict[int, int]]]]
    ...
    def call_simulate_qiskit(
        self, circuit: QuantumCircuit
    ) -> List[Dict[str, Dict[int, int]]]:
        counts = {}
        count = self.simulate_qiskit(circuit, "Qiskit - Not transpiled")
        counts["Q-n"] = count
        transpiled_circuit_qiskit = self.transpile_qiskit(circuit)
        count = self.simulate_qiskit(transpiled_circuit_qiskit, "Qiskit - Transpiled")
        counts["Q-t"] = count
        return counts

    def call_simulate_rigetti(
        self, circuit: QuantumCircuit
    ) -> List[Dict[str, Dict[int, int]]]:
        counts = {}
        program = self.convert_to_pyquil(circuit)
        # print(program)
        count = self.simulate_pyquil(program, "Rigetti - Not transpiled")
        counts["R-n"] = count
        transpiled_circuit_pyquil = self.transpile_pyquil(circuit)
        count = self.simulate_pyquil(transpiled_circuit_pyquil, "Rigetti - Transpiled")
        counts["R-t"] = count
        return counts
