...
class ConversionHandler:
    def __init__(self, converter: ConverterInterface.__class__):
        self.converter: ConverterInterface.__class__ = converter
    ...
    def export_language(self, circuit: QuantumCircuit) -> Tuple[Any, Any, Any]:
        (converted_circuit, qreg_mapping, creg_mapping) = self.export_circuit(circuit)
        return (
            self.converter.circuit_to_language(converted_circuit),
            qreg_mapping,
            creg_mapping,
        )

    def export_circuit(self, circuit: QuantumCircuit, recursive: bool = False):
        # Check if conversion needs to be done manually
        if self.converter.has_internal_export:
            return self.converter.export_circuit(circuit), {}, {}
        else:
            ...
            for instr in circuit.data:
                ...
                if isinstance(qiskit_gate, qiskit_circuit_library.Gate):
                    ...
                    subcircuit = self.export_circuit(
                        instr[0].decompositions[0], recursive=True
                    )[0]
    ...
    def _handle_gate_export(self, converter, qiskit_gate, qubits) -> None:
        ...
        if converter.is_control_capable:
            ...
            if not (self._in_mappings(converter, qiskit_gate_class_name, "g")):
                ...
                if isinstance(qiskit_gate, ControlledGate) and qiskit_gate.base_gate:
                    ...
                    # if base gate has only replacement entry, check if it can be controlled
                    if (
                        converter.name in gate_mapping_qiskit[qiskit_gate_class_name]
                        and gate_mapping_qiskit[qiskit_gate_class_name][converter.name]
                        and (
                            "g"
                            in gate_mapping_qiskit[qiskit_gate_class_name][
                                converter.name
                            ]
                            or (
                                "r"
                                ...
                                in inspect.getfullargspec(
                                    gate_mapping_qiskit[qiskit_gate_class_name][
                                        converter.name
                                    ]["r"]
                                )[0]
                            )
        ...
        if qiskit_gate_class_name in gate_mapping_qiskit:
            ...
            for i, param in enumerate(params):
                ...
                elif isinstance(param, qiskit_Parameter_expression):
            ...
            if self._in_mappings(converter, qiskit_gate_class_name, "r"):
                replacement_function = self._get_gate(
                    converter, qiskit_gate_class_name, "r"
                )
                if is_controlled:
                    params.append(is_controlled)
        ...
        try:
            ...
            name = (
                qiskit_gate_class_name
                if qiskit_gate_class_name
                else "unitary" + str(id(qiskit_gate))
            )
    ...
    def _in_mappings(
        self, converter: ConverterInterface, qiskit_gate_class_name: str, key: str
    ) -> bool:
        ...
        if qiskit_gate_class_name in gate_mapping_qiskit:
            ...
            if (
                converter.name in gate_mapping_qiskit[qiskit_gate_class_name]
                and gate_mapping_qiskit[qiskit_gate_class_name][converter.name]
                and key in gate_mapping_qiskit[qiskit_gate_class_name][converter.name]
            ):
                return True
            return False
        return False
